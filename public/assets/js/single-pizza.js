...
function getPizza() {
 ...
 //get pizzaInfo to display on the page we are fetching 
 fetch(`/api/pizzas/${pizzaId}`)
 .then(response => {
  // check for a 4xx or 5xx error from server always include error handling 
  if (!response.ok) {
...
.catch(err => {
  ...
  //The window history method lets us control browser state as long as session as a previous page it will act like use clicked back
  window.history.back();
});
}
...
function handleNewCommentSubmit(event) {
  ...
  // This is calling the API we created 
...
// we have to call the function created at the top of the page to get pizza stuff 
getPizza(); 